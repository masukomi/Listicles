unit module Listicles:ver<1.6.0>:auth<masukomi (masukomi@masukomi.org)>;
...
augment class Array {
	...
	#| Ex. to see if all numbers are even $my_array.all-are(-> $x {$x mod 2 == 0})
	method all-are(&truthy_function) returns Bool {
		self.List.all-are(&truthy_function);
	}

	#| Returns True if the function passed in returns true for any item.
	#| Function must take a single paramater and return True or False
	#| Ex. to see if any numbers are even $my_array.any-are(-> $x {$x mod 2 == 0})
	method any-are(&truthy_function) returns Bool {
		self.List.any-are(&truthy_function);
	}
...
augment class Seq {
	...
	#| Returns True if the function passed in returns true for all items.
	#| Function must take a single paramater and return True or False
	#| Ex. to see if all numbers are even $my_seq.all-are(-> $x {$x mod 2 == 0})
	method all-are(&truthy_function) returns Bool {
		for self -> $element {
	...
	#| Returns True if the function passed in returns true for any item.
	#| Function must take a single paramater and return True or False
	#| Ex. to see if any numbers are even $my_seq.any-are(-> $x {$x mod 2 == 0})
	method any-are(&truthy_function) returns Bool {
		for self -> $element {
...
augment class List {
	#| Returns True if the function passed in returns true for all items.
	#| Function must take a single paramater and return True or False
	#| Ex. to see if all numbers are even $my_list.all-are(-> $x {$x mod 2 == 0})
	method all-are(&truthy_function) returns Bool {
		for self -> $element {
			return False unless &truthy_function($element);
		}
	...
	#| Returns True if the function passed in returns true for any item.
	#| Function must take a single paramater and return True or False
	#| Ex. to see if any numbers are even $my_list.any-are(-> $x {$x mod 2 == 0})
	method any-are(&truthy_function) returns Bool {
		for self -> $element {
			return True if &truthy_function($element);
		}
